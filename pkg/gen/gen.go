...
const testingTemplate = `// Code generated by aocgen; DO NOT EDIT.
	package year{{.Year}}
...
func NewTests(year int, day int) {
	path := fmt.Sprintf("./pkg/year%d", year)
	fileName := fmt.Sprintf("%s/day%s_test.go", path, FormatDay(day))

	tests := fmt.Sprintf(`func TestDay%sPartA(t *testing.T) {
		Init()
	...
	func TestDay%sPartB(t *testing.T) {
		Init()
		input := strings.Split(example, "\n")
		p := aoc.NewPuzzle(%d, %d)
	...
	`, FormatDay(day), year, day, FormatDay(day), year, day)

	tmpl := codegen.MustParse(testingTemplate)
	if err := tmpl.CreateFile(fileName, map[string]interface{}{
		"Year":  year,
		"Tests": tests,
	}); err != nil {
		logrus.Fatal(errors.Wrap(err, "could not generate file"))
	}
